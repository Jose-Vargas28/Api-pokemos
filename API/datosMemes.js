...
const obtenerMemes = async ()=>{
    ...
    response.data.memes.slice(0,20).forEach(meme => {
...
const obtenerGif= async()=>{
    const request = await fetch("http://api.giphy.com/v1/gifs/search?api_key=lerx9FDWD5PH78M207Cj95ac4KTt8G3v&q=goku&limit=3")
    const response = await request.json()
